...
if ( ! class_exists( 'SP_WP_TABS_Options' ) ) {
	...
	class SP_WP_TABS_Options extends SP_WP_TABS_Abstract {
		...
		public function ajax_save() {
			...
			if ( ! $result ) {
				wp_send_json_error( array( 'error' => esc_html__( 'Error while saving.', 'wp-tabs' ) ) );
			} else {
		...
		public function set_options( $ajax = false ) {
			...
			if ( wp_verify_nonce( $nonce, 'wptabspro_options_nonce' ) ) {
				...
				if ( ! $ajax && ! empty( $response['wptabspro_import_data'] ) ) {
					...
					$importing    = true;
					$this->notice = esc_html__( 'Success. Imported backup options.', 'wp-tabs' );
				...
				if ( ! empty( $transient['reset'] ) ) {
					...
					$this->notice = esc_html__( 'Default options restored.', 'wp-tabs' );
					...
					$this->notice = esc_html__( 'Default options restored for only this section.', 'wp-tabs' );
				...
				if ( empty( $this->notice ) ) {
					$this->notice = esc_html__( 'Settings saved.', 'wp-tabs' );
				}
		...
		public function add_options_html() {
			...
			echo ( $has_nav && $this->args['show_all_options'] ) ? '<div class="wptabspro-expand-all" title="' . esc_html__( 'show all options', 'wp-tabs' ) . '"><i class="fa fa-outdent"></i></div>' : '';

			echo ( $this->args['show_search'] ) ? '<div class="wptabspro-search"><input type="text" name="wptabspro-search" placeholder="' . esc_html__( 'Search option(s)', 'wp-tabs' ) . '" autocomplete="off" /></div>' : '';
			if ( $show_buttons ) {
				echo '<div class="wptabspro-buttons">';
				echo '<input type="submit" name="' . esc_attr( $this->unique ) . '[_nonce][save]" class="button button-primary wptabspro-top-save wptabspro-save' . esc_attr( $ajax_class ) . '" value="' . esc_html__( 'Save', 'wp-tabs' ) . '" data-save="' . esc_html__( 'Saving...', 'wp-tabs' ) . '">';
				echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="wptabspro_transient[reset_section]" class="button button-secondary wptabspro-reset-section wptabspro-confirm" value="' . esc_html__( 'Reset Section', 'wp-tabs' ) . '" data-confirm="' . esc_html__( 'Are you sure to reset this section options?', 'wp-tabs' ) . '">' : '';
				echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="wptabspro_transient[reset]" class="button wptabspro-warning-primary wptabspro-reset-all wptabspro-confirm" value="' . esc_html__( 'Reset All', 'wp-tabs' ) . '" data-confirm="' . esc_html__( 'Are you sure to reset all options?', 'wp-tabs' ) . '">' : '';
				echo '</div>';
			}
			...
			foreach ( $this->pre_sections as $section ) {
				...
				if ( ! empty( $section['fields'] ) ) {
					...
					echo '<div class="wptabspro-no-option wptabspro-text-muted">' . esc_html__( 'No option provided by developer.', 'wp-tabs' ) . '</div>';
