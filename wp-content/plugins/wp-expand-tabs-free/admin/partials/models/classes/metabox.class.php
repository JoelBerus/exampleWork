	...
	class SP_WP_TABS_Metabox extends SP_WP_TABS_Abstract {
		...
		public function add_meta_box_content( $post, $callback ) {
			...
			if ( ! empty( $this->args['show_restore'] ) ) {
				...
				echo '<span class="button wptabspro-button-restore">' . esc_html__( 'Restore', 'wp-tabs' ) . '</span>';
				echo '<span class="button wptabspro-button-cancel">' . sprintf( '<small>( %s )</small> %s', esc_html__( 'update post for restore ', 'wp-tabs' ), esc_html__( 'Cancel', 'wp-tabs' ) ) . '</span>';
				echo '</label>';
				echo '</div>';
