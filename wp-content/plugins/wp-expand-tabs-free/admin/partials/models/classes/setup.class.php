	...
	class SP_WP_TABS {
			...
			if ( 'sp_wp_tabs' === $the_current_post_type ) {
				...
				wp_localize_script(
					...
					array(
						...
						'i18n'          => array(
							// global localize.
							'confirm'             => esc_html__( 'Are you sure?', 'wp-tabs' ),
							'reset_notification'  => esc_html__( 'Restoring options.', 'wp-tabs' ),
							'import_notification' => esc_html__( 'Importing options.', 'wp-tabs' ),

							// chosen localize.
							'typing_text'         => esc_html__( 'Please enter %s or more characters', 'wp-tabs' ),// phpcs:ignore
							'searching_text'      => esc_html__( 'Searching...', 'wp-tabs' ),
							'no_results_text'     => esc_html__( 'No results match', 'wp-tabs' ),
						),
					)
				);
		...
		public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
			...
			if ( ! empty( $field['_notice'] ) ) {
				...
				$field            = array();
				$field['content'] = sprintf( esc_html__( 'Ooops! This field type (%s) can not be used here, yet.', 'wp-tabs' ), '<strong>' . $field_type . '</strong>' ); // phpcs:ignore
				$field['type']    = 'notice';
				$field['style']   = 'danger';
			...
			if ( ! empty( $field_type ) ) {
				...
				if ( ! empty( $field['title'] ) ) {
					echo '<div class="wptabspro-title">';
					echo '<h4>' . wp_kses_post( $field['title'] ) . '</h4>';
					echo ( ! empty( $field['title_help'] ) ) ? '<div class="wptabspro-help"><span class="wptabspro-help-text">' . wp_kses_post( $field['title_help'] ) . '</span><i class="fa fa-question-circle"></i></div>' : '';
					echo ( ! empty( $field['subtitle'] ) ) ? '<div class="wptabspro-text-subtitle">' . wp_kses_post( $field['subtitle'] ) . '</div>' : '';
					echo '</div>';
				}
				...
				if ( class_exists( $classname ) ) {
					...
					echo '<p>' . esc_html__( 'This field class is not available!', 'wp-tabs' ) . '</p>';
				}
			} else {
				echo '<p>' . esc_html__( 'This type is not found!', 'wp-tabs' ) . '</p>';
			}
