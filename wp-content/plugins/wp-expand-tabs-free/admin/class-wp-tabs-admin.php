...
class WP_Tabs_Admin {
	...
	public function duplicate_wp_tabs() {
		...
		if ( ! ( isset( $_GET['post'] ) || isset( $_POST['post'] ) || ( isset( $_REQUEST['action'] ) && 'sp_duplicate_tabs' === $_REQUEST['action'] ) ) ) {
			wp_die( esc_html__( 'No tabs to duplicate has been supplied!', 'wp-tabs' ) );
		}
		...
		*/
		if ( isset( $post ) && null !== $post ) {
			...
			*/
			...
			wp_die( esc_html__( 'Tabs creation failed, could not find original tabs: ', 'wp-tabs' ) . esc_html( $post_id ) );
		}
	}
	...
	public function sp_duplicate_tabs_link( $actions, $post ) {
		if ( current_user_can( 'edit_posts' ) && 'sp_wp_tabs' === $post->post_type ) {
			$actions['duplicate'] = '<a href="' . wp_nonce_url( 'admin.php?action=sp_duplicate_tabs&post=' . $post->ID, basename( __FILE__ ), 'sp_duplicate_tabs_nonce' ) . '" rel="permalink">' . __( 'Duplicate', 'wp-tabs' ) . '</a>';
		}
		return $actions;
	}
