...
class WP_Tabs_Admin_Menu {
	...
	public function sptpro_plugin_action_links( $links, $file ) {
		...
		if ( WP_TABS_BASENAME === $file ) {
			...
			$new_links = array(
				sprintf( '<a href="%s">%s</a>', admin_url( 'post-new.php?post_type=sp_wp_tabs' ), __( 'Add Tab Group', 'wp-tabs' ) ),
			);
			$links[]   = '<a href="https://shapedplugin.com/plugin/wp-tabs-pro/?ref=1" style="color: #35b747; font-weight: 700;">' . __( 'Go Premium!', 'wp-tabs' ) . '</a>';
	...
	public function sptpro_review_text( $text ) {
		...
		if ( 'sp_wp_tabs' === get_post_type() || ( 'sp_wp_tabs_page_tabs_settings' === $screen->id ) || ( 'sp_wp_tabs_page_tabs_help' === $screen->id ) ) {
			...
			$text = sprintf( wp_kses_post( 'If you like <strong>WP Tabs</strong>, please leave us a <a href="%s" target="_blank">&#9733;&#9733;&#9733;&#9733;&#9733;</a> rating. Your Review is very important to us as it helps us to grow more. ', 'wp-tabs' ), esc_url( $url ) );
		}
