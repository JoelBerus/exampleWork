...
class WP_Tabs_CPT {
	...
	public function sptpro_post_type() {
		...
		$labels = apply_filters(
			...
			array(
				'name'               => esc_html_x( 'Tab Groups', 'wp-tabs' ),
				'singular_name'      => esc_html_x( 'Tabs', 'wp-tabs' ),
				'add_new'            => esc_html__( 'Add New', 'wp-tabs' ),
				'add_new_item'       => esc_html__( 'Add New Tab Group', 'wp-tabs' ),
				'edit_item'          => esc_html__( 'Edit Tab Group', 'wp-tabs' ),
				'new_item'           => esc_html__( 'New Tab Group', 'wp-tabs' ),
				'view_item'          => esc_html__( 'View Tab Group', 'wp-tabs' ),
				'search_items'       => esc_html__( 'Search Tabs', 'wp-tabs' ),
				'not_found'          => esc_html__( 'No Tab Group found.', 'wp-tabs' ),
				'not_found_in_trash' => esc_html__( 'No Tab Group found in trash.', 'wp-tabs' ),
				'parent_item_colon'  => esc_html__( 'Parent Item:', 'wp-tabs' ),
				'menu_name'          => esc_html__( 'WP Tabs', 'wp-tabs' ),
				'all_items'          => esc_html__( 'Tab Groups', 'wp-tabs' ),
			)
		);
	...
	public function sptpro_updated_messages( $messages ) {
		...
		$messages['sp_wp_tabs'] = array(
			...
			1  => sprintf( __( 'Tabs updated.', 'wp-tabs' ) ),
			2  => '',
			3  => '',
			4  => __( ' updated.', 'wp-tabs' ),
			...
			6  => sprintf( __( 'Tabs published.', 'wp-tabs' ) ),
			7  => __( 'Tabs saved.', 'wp-tabs' ),
			8  => sprintf( __( 'Tabs submitted.', 'wp-tabs' ) ),
			9  => sprintf( wp_kses_post( 'Tabs scheduled for: <strong>%1$s</strong>.', 'wp-tabs' ), date_i18n( __( 'M j, Y @ G:i', 'wp-tabs' ), strtotime( $post->post_date ) ) ),
			10 => sprintf( __( 'Tabs draft updated.', 'wp-tabs' ) ),
		);
		return $messages;
	}
	...
	public function sptpro_admin_column( $columns ) {
		return array(
			...
			'title'     => __( 'Name', 'wp-tabs' ),
			'shortcode' => __( 'Shortcode', 'wp-tabs' ),
			'date'      => __( 'Date', 'wp-tabs' ),
		);
	}
